...
toggleP = function(p){
  ...
  var selectedPrimes = []; 
  var selectedPrimesSelector = []; 
  $(".prime-selector.prime-selected").each(function(){
    selectedPrimes.push($(this).val());
    selectedPrimesSelector.push(".p"+$(this).val());
  });
  $("rect.active").attr("data-level", "2");
  var colSelector = "g"+selectedPrimesSelector.join("");
  var cellSelector = selectedPrimesSelector.join(",");
  if (selectedPrimes.length > 0) {
    $(colSelector).children(cellSelector).filter(".active").attr("data-level", "4");
  }
  updateQualifiers(selectedPrimes);
}

updateQualifiers = function(selectedPrimes) {
  var val = 1;
  var selectedPrimesSelector = []; 
  selectedPrimes.forEach((p) => {
    val*=p;
    selectedPrimesSelector.push(".p"+p);
  });
...
//    htmlStr += `<input type="button" class="${"prime-selector" + selectedPrimes.includes(p)?" prime-selected":""}" value="${p}" onclick="toggleP(${p})"/>`;
  });
  htmlStr += "</div>";
